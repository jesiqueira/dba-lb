...
- Esse é o maior causador de bloqueios, e vimoa isso AO VIVO no teste anterior.

# 🔥 O que é WAL?

### WAL = Write Ahead Log - (Registro de Escrita Antecipada)

Ele é o diário de tudo que muda no banco.
Antes de qualquer mudança ir para os arquivos reais, o PostgreSQL primeiro grava no WAL.

👉 Isso garante durabilidade (a letra D do ACID)

👉 E permite recuperação do banco após falhas
...
```sql
UPDATE clientes SET nome = 'José' WHERE id = 10;
```

### O PostgreSQL faz:

1️⃣ Gera o WAL record

- Um registro binário descrevendo a mudança (ex: linha alterada, página alterada etc.)

2️⃣ Grava esse registro no arquivo WAL

- Grava no disco rapidamente (escrever sequencial é super rápido).

3️⃣ Depois aplica a mudança no buffer de dados

- Os blocos reais da tabela são alterados na memória.

4️⃣ Mais tarde os buffers de dados vão para o disco

- (não é imediato — é gerenciado pelo background writer e checkpoints).

Ou seja:

- ➡️ Primeiro escreve o WAL
- ➡️ Depois o dado real
...
Cada arquivo tem 16 MB (padrão).

# 🎯 PARA QUE SERVE?

### ✔ 1. Garantir recuperação após queda (crash recovery)
Se o servidor cair, o Postgres:
 - lê o banco no disco
 - verifica alterações incompletas
 - reaplica o WAL para reconstruir o estado consistente

### ✔ 2. Replicação física (streaming replication)
Os WALs são enviados para réplicas.

Não é enviado “SQL” para o slave, mas sim WAL binário.

A réplica aplica esses WALs e fica idêntica ao master.

### ✔ 4. Checkpoints
Um checkpoint é o momento que o PostgreSQL faz:

- escreve no disco todos os buffers sujos (dirty pages)
- grava um ponto no WAL indicando “tudo antes daqui já está no disco”

Isso evita que na recuperação o Postgres tenha que ler milhares de WALs.

# 📌 COMPONENTES IMPORTANTES DO WAL

### 1. WAL Writer
Processo que grava WAL em disco periodicamente.
### 2. Background writer
Escreve páginas modificadas (buffers sujos) nos arquivos reais.
### 3. Checkpointer
Ponto onde tudo sincroniza.
### 4. Archiver
Se você habilitar archive_mode, cada WAL finalizado é arquivado para uso em backups/PITR.

# 🎯 PARÂMETROS IMPORTANTES DO WAL
...
### Principais:
### 🔸 wal_level
Define “quanto” de WAL é gerado.
- minimal
- replica (padrão)
- logical (para replication lógica e Debezium)

### 🔸 max_wal_size
Tamanho máximo que o diretório pg_wal pode crescer antes de forçar um novo checkpoint.

### 🔸 checkpoint_timeout
Tempo máximo entre checkpoints.

### 🔸 synchronous_commit
Controla quando o PostgreSQL considera uma gravação confirmada.

Valores:

- on (mais seguro)
- off (rápido mas arriscado)

# 🔥 EXEMPLO SIMPLES DE FLUXO WAL
Você faz:
```sql 
INSERT INTO logs VALUES ('teste');
```
Postgres faz na ordem:

- Cria WAL record
- Grava no arquivo WAL
- Retorna sucesso para a aplicação
- Só depois atualiza o arquivo da tabela
- No checkpoint tudo é sincronizado

# 🧠 MAS... POR QUE NÃO ESCRVER DIRETO NO ARQUIVO DA TABELA?
Porque:

### ✔ escrever no WAL é sequencial
muito rápido para disco físico e SSD.
### ✔ escrever na tabela é aleatório
bem mais lento.
### ✔ escrever primeiro no WAL é mais seguro
se cair energia no meio da escrita da tabela, o WAL reconstruirá tudo.
### ✔ WAL permite replicação
os slaves aplicam o WAL para ficar em sync.

# 📌 COMANDOS ÚTEIS QUANDO VOCÊ ESTUDA WAL
Ver todos os parâmetros do WAL:
```sql
SELECT name, setting 
FROM pg_settings 
WHERE name LIKE 'wal%';
...
| Conceito | Explicação |
| ---------|------------|
| WAL  | Log de todas as mudanças, gravado antes do dado real|
| Garantia | Durabilidade (ACID)|
| Usos | Recuperação, replicação, PITR|
| Onde fica | pg_wal/ |
| Por que existe | Escrever log sequencial é muito mais rápido e seguro|
